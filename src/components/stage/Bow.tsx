  ...
  return (
    ...
    >
      <div className="sealed-guess mx-auto max-w-[62ch] p-5 text-left">
        <p className="font-display text-[18px] text-gold-accent">
          The sealed guess
        </p>
        <p className="mt-2 text-paper-muted">
          Before you started, a guess was sealed at what you would give on
          average. The number is{" "}
          <span className="font-display text-paper">7</span>.
        </p>
        {opened && (
          <p className="mt-3 text-paper">
            {result.avg === null ? (
              <>
                You scored nothing at all. The envelope opens on an empty room.
                The guess loses.
              </>
            ) : (
              <>
                You averaged{" "}
                <span className="font-display">{result.avg.toFixed(1)}</span>.
                That rounds to{" "}
                <span className="font-display">{result.rounded}</span>.{" "}
      ...
      <p className="mt-4 text-paper-muted">
        If any of it was useful, the rest is a conversation.
      </p>

      <div className="mt-8 flex flex-wrap justify-center gap-4">
        <a
          className="poster-btn font-display"
